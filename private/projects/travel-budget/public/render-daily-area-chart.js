function renderDailyAreaChart(data, rootElement, config) {
    ...
    for (var date_idx = 0; date_idx < dates.length; date_idx++) {
        ...
        for (var i = 0; i < date_map[dates[date_idx]].y.length; i++) {
            stack_data2[i].push({x:date_idx, y:date_map[dates[date_idx]].y[i], details:date_map[dates[date_idx]].details[i]})
        };
    };

    var average_stack_data = [];
    for (var i = 0; i < labels.length; i++) {
        average_stack_data.push([ { x: dates.length, y: d3.sum(dates, function(d) { return date_map[d].y[i] }) / dates.length }]);
    }

    console.log(average_stack_data);
    ...
    var w = config.width,
        ...
        xScale = d3.scale.linear().domain(xExtent).range([inner_pad, w-200-inner_pad]), // evidently the stack layout requires a numerical x axis
        xScaleDates = d3.time.scale().domain(xExtentDates).range([inner_pad, w-200-inner_pad]),
        xScaleBand = d3.scale.ordinal().rangeRoundBands([inner_pad, w-200-inner_pad], .1),
...
/*
  vis.selectAll(".barChart")
    ...
    .enter().append("svg:g")
       .selectAll(".bars")
       .data(function(d) {
       console.log(date_map[d]);return date_map[d]; })
    .enter().append("rect")
      .attr("width", xScaleBand.rangeBand())
      .attr("y", function(d) { console.log(d); return yScale(d.y); })
      .attr("x", function(d) { return xScale(d.x); })
      .attr("height", function(d) { return yScale(5) - yScale(d.y); })
      .style("fill", function(d,i) { return legend_item[i].color })
*/

  vis.selectAll(".barChart")
      .data(stack_layout_data)
    .enter().append("svg:g")
       .style("fill", function(d,i) { console.log(i); return legend_item[i % legend_item.length].color })
       .selectAll(".bars")
       .data(function(d) {
...
/*
    vis.selectAll(".areaChart")
        .attr('class', 'areaChart')
        .data(stack_layout_data)
        .enter()
            .append("path")
            .attr('class', 'areaChartPath')
            .style("fill", function(d,i) { return legend_item[i].color })
            .attr("d", area);
/*            .on('click', function (d,i) {
            ...
            .call(d3.helper.tooltip(function (d,i) {
                ...
                var date_idx = Math.floor(xScale.invert(m[0]))+1;
                var date = xScaleDates.invert(m[0]);
                var tooltip_html = '<h2> '+ date + '</h2><br/>';//<table>';
...
*/
    ...
    for (var legend_item_num = 0; legend_item_num < legend_item.length; legend_item_num++)
    {
        legend
            .append("svg:rect")
            .attr("x", 0)
            .attr("y", 30 + legend_item_num*15)
            .attr("stroke", legend_item[legend_item_num].color)
            .attr("fill", legend_item[legend_item_num].color)
            .attr("height", 5)
            .attr("width", 25);
        ...
        legend
            ...
            .attr("x", 30)
            .attr("y", 35 + legend_item_num*15)
            .text(legend_item[legend_item_num].name);
     };
   ...
   series_label.data(average_layout_data)
        .enter()
            .append("svg:text")
  ...
  function resize() {
    ...
    xScale.range([inner_pad, width-200-inner_pad]);
    xScaleDates.range([inner_pad, width-200-inner_pad]);
